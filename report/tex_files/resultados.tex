...
\begin{itemize}
	\item \textbf{Cluster 1 (Activación Inmune):} Agrupa genes como \textit{RNF125} e \textit{IRAK1}. El análisis (Figura \ref{fig:Enrichment_C1}) mostró un enriquecimiento significativo en la "activación de la vía de señalización inmune innata". Es el motor de arranque de la respuesta inflamatoria.
	\item \textbf{Cluster 2 (Respuesta Leucocitaria):} Contiene a \textit{IL10} y genes HLA. Se asoció principalmente a la "inmunidad mediada por leucocitos" y la "regulación del proceso efector inmune", indicando un rol en la presentación de antígenos y comunicación celular (Figura \ref{fig:Enrichment_C2}).
	
	\item \textbf{Cluster 3 (Firma de Interferón):} Este fue el hallazgo más distintivo. Como se observa en la Figura \ref{fig:Enrichment_C3}, los genes de este grupo (\textit{IFIH1}, \textit{STING1}) están casi exclusivamente dedicados a la "respuesta a interferón de tipo I" y la "defensa antiviral".
	
	\item \textbf{Cluster 4 (Senescencia Nuclear):} A diferencia de los anteriores, este grupo (Figura \ref{fig:Enrichment_C4}) se enriqueció en procesos de "senescencia celular" y "organización de la envoltura nuclear", dirigidos por genes como \textit{LMNA} y \textit{ZMPSTE24}.
	\end{itemize}
